...
| Timestamp      | 4            | `HAL_GetTick()` in ms             |
| Payload        | N            | Telemetry payload                 |
| CRC16          | 2            | CRC-16/CCITT-FALSE                |

Total frame size = `12 + payload_length + 2`
...
|--------|---------------|---------|------------|
| 0      | `filt_adc`    | `uint16`| ADC counts |
| 2      | `temp_c_x100` | `int16` | °C × 100   |
...
- New payload fields should increase `payload_length`
- New message types should use a new `msg_type` value
